...
def test_parse_types_of_business_other_skips_repeated_header_inside_body() -> None:
    """FINRA renders Item 12B across two pages on big firms (CRD 19585
    HSBC, observed 2026-05-06): the section header repeats at the top of
    the second page after a blank line, then the body continues. The
    one-shot ``re.sub(..., count=1)`` strips the leading header but the
    second occurrence used to land in the joined output as
    ``"Other Types of Business This firm does..."``. Skip any line that
    matches the section header on its own, regardless of position."""
    section = (
        "Other Types of Business\n"
        "\n"
        "Other Types of Business\n"
        "This firm does effect transactions in commodities, commodity "
        "futures, or commodity options.\n"
        "This firm does engage in other non-securities business.\n"
    )
    parsed = _parse_types_of_business_other(section)
    assert parsed is not None
    assert not parsed.startswith("Other Types of Business")
    assert parsed.startswith("This firm does effect transactions")


def test_parse_types_of_business_other_filters_kerning_collapsed_page_header() -> None:
    """pdfplumber renders the per-page banner ``www.finra.org/brokercheck
    User Guidance`` with kerning gaps that collapse char-by-char into the
    Item 12B body when the section straddles a page boundary. CRD 31194
    (RBC Capital Markets, observed 2026-05-06) produced::

        wOwtwh.efinr rTa.yorpge/bsro okefr cBhuecskiness User Guidance

    The ``_HEADER_BOILERPLATE`` regex in ``_normalize`` strips the
    non-kerned form but the URL no longer matches once the kerning
    interleaves another header into it. The one fingerprint that
    survives the kerning intact is the trailing ``User Guidance`` —
    drop any body line that contains it."""
    section = (
        "Other Types of Business\n"
        "\n"
        "wOwtwh.efinr rTa.yorpge/bsro okefr cBhuecskiness User Guidance\n"
        "This firm does effect transactions in commodities.\n"
    )
    parsed = _parse_types_of_business_other(section)
    assert parsed == "This firm does effect transactions in commodities."


def test_parse_types_of_business_other_filters_kerning_collapsed_page_footer() -> None:
    """When the per-page copyright footer
    ``©2026 FINRA. All rights reserved. Report about <FIRM>. <pageno>``
    straddles a page break inside Item 12B, pdfplumber kerns the U+00A9
    copyright glyph and the rest of the footer char-by-char into the
    first content line. CRD 8174 (UBS Financial Services, observed
    2026-05-06) produced::

        T©h20is2 6fi rFmIN RdAo.e Asll erigffhetsc tr etsraernvsead.

    The existing ``ln.startswith('©')`` check only handles the
    untouched footer line; once the copyright glyph migrates a few
    characters into a content line under kerning, only a substring
    fingerprint catches it. Form BD Item 12B is regulator prose — no
    firm puts a copyright symbol in their business description — so
    this is safe."""
    section = (
        "Other Types of Business\n"
        "T©h20is2 6fi rFmIN RdAo.e Asll erigffhetsc tr etsraernvsead.\n"
    ...
    parsed = _parse_types_of_business_other(section)
    assert parsed == "This firm does engage in other non-securities business."


def test_parse_types_of_business_other_returns_none_when_only_boilerplate() -> None:
    """Safety net for legacy/terminated firms whose Item 12B section is
    pdfplumber-corrupt end-to-end: every body line is either a repeated
    header or kerned page boilerplate. After filtering, the body
    collapses to nothing — return None instead of stamping junk on the
    column."""
    section = (
        "Other Types of Business\n"
        "Other Types of Business\n"
        "wOwtwh.efinr rTa.yorpge/bsro okefr cBhuecskiness User Guidance\n"
        "T©h20is2 6fi rFmIN RdAo.e Asll erigffhetsc.\n"
    )
    assert _parse_types_of_business_other(section) is None
