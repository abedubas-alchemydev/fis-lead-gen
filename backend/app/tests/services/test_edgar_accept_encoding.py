...
:class:`EdgarService` builds five separate ``headers`` dicts:
...
* ``fetch_last_filing_for_cik`` — per-CIK submissions JSON for the
  refresh-all orchestrator's filings sub-pipeline.
* ``list_all_filings_for_cik`` — per-CIK submissions JSON for the
  master-list filing-history pagination endpoint.
...
of the five lines fails this test before it can ship.
"""
...
def test_edgar_service_module_pins_accept_encoding_identity_in_five_places() -> None:
    source = inspect.getsource(edgar_module)
    occurrences = source.count('"Accept-Encoding": "identity"')
    assert occurrences == 5, (
        "EdgarService must pin Accept-Encoding: identity in all five header "
        "dicts (_fetch_via_company_search, fetch_records_for_sec_numbers, "
        "_ensure_bulk_submissions_zip, fetch_last_filing_for_cik, "
        "list_all_filings_for_cik). Found "
        f"{occurrences} occurrence(s); expected 5. SEC EDGAR's Cloudflare "
