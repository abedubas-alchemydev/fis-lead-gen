...
:class:`EdgarService` builds four separate ``headers`` dicts:
...
of the four lines fails this test before it can ship.
"""
...
def test_edgar_service_module_pins_accept_encoding_identity_in_four_places() -> None:
    source = inspect.getsource(edgar_module)
    occurrences = source.count('"Accept-Encoding": "identity"')
    assert occurrences == 4, (
        "EdgarService must pin Accept-Encoding: identity in all four header "
        "dicts (_fetch_via_company_search, fetch_records_for_sec_numbers, "
        "_ensure_bulk_submissions_zip, fetch_last_filing_for_cik). Found "
        f"{occurrences} occurrence(s); expected 4. SEC EDGAR's Cloudflare "
