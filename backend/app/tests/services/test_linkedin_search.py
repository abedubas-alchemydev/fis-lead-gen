...
Plus: non-profile URLs filtered out, name mismatch rejected, serper->serpapi
fallback, and silent no-op when neither search key is configured.

respx mocks both search backends; no real quota is burned.
"""
...
from app.services.serper import _SERPER_SEARCH_URL


@pytest.fixture
def keys(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "serper_api_key", "test-serper")
    monkeypatch.setattr(settings, "serpapi_api_key", "test-serpapi")
    monkeypatch.setattr(settings, "contact_discovery_timeout", 2.0)


def _serper(organic: list[dict]) -> httpx.Response:
...
async def test_org_confirmed_match(keys: None) -> None:
    respx.post(_SERPER_SEARCH_URL).mock(
        return_value=_serper([
            _hit(
...
@pytest.mark.asyncio
@respx.mock
async def test_serper_error_falls_through_to_serpapi(keys: None) -> None:
    respx.post(_SERPER_SEARCH_URL).mock(return_value=httpx.Response(500))
    respx.get(_SERPAPI_SEARCH_URL).mock(
        return_value=httpx.Response(200, json={"organic_results": [
            {"link": "https://www.linkedin.com/in/jane-doe-vanguard",
             "title": "Jane Doe - Vanguard | LinkedIn",
             "snippet": "MD at Vanguard."},
        ]})
    )
    provider = LinkedInSearchProvider()
    ...
    assert result is not None
    assert result.linkedin_url == "https://www.linkedin.com/in/jane-doe-vanguard"


@pytest.mark.asyncio
@respx.mock
async def test_no_keys_returns_none(monkeypatch: pytest.MonkeyPatch) -> None:
