...
def test_has_scorable_data_empty_firm_returns_false():
    """A firm with no financials, no clearing classification, and no
    clearing-partner string has nothing meaningful to score on. The
    bulk runner skips these as ``skipped_no_data`` rather than
    producing a default-driven 0.x score."""
    assert has_scorable_data(_firm()) is False


@pytest.mark.parametrize(
    "overrides",
    [
        {"latest_net_capital": Decimal("1000")},
        {"clearing_classification": "fully_disclosed"},
        {"clearing_classification": "needs_review"},
        {"current_clearing_partner": "Pershing LLC"},
        {"current_clearing_partner": "Some Random Bank"},
...
def test_has_scorable_data_any_signal_returns_true(overrides):
    assert has_scorable_data(_firm(**overrides)) is True


def test_has_scorable_data_ignores_filing_recency_and_branch_count():
    """``last_filing_date`` and ``branch_count`` alone aren't enough —
    they shape the recency/size buckets but the brief's skip rule is
    explicitly about financials, classification, and clearing partner."""
    firm = _firm(last_filing_date=date(2026, 1, 1), branch_count=10)
    assert has_scorable_data(firm) is False


# ── score_broker_dealers (bulk runner) ──────────────────────────────
#
# The DB I/O helpers (``_load_target_firms`` / ``_load_default_weights``
# / ``_load_competitor_lookup``) are exercised end-to-end by integration
# runs against real Postgres. These tests pin down the orchestration
# logic — NULL-priority filter, ``--all`` mode, dry-run semantics, and
# the ``skipped_no_data`` skip rule — without spinning up a database.


@dataclass
class _FakeFirm:
    """Mutable stand-in for ``BrokerDealer`` so tests can assert on
    ``lead_score`` / ``lead_priority`` writes."""

    id: int
    current_clearing_partner: str | None = None
    clearing_classification: str | None = None
    latest_net_capital: Decimal | None = None
...
class _FakeSession:
    ...
    def add(self, _entity) -> None:  # pragma: no cover - trivial
        return None
...
def _hot_firm():
    return _FakeFirm(
        ...
        current_clearing_partner="Pershing LLC",
        clearing_classification="fully_disclosed",
        latest_net_capital=Decimal("50000000"),
        last_filing_date=date(2026, 4, 1),
        branch_count=30,
    )
...
@pytest.fixture
def _empty_firm():
    return _FakeFirm(id=3)


def _patch_loaders(monkeypatch, *, firms, weights, lookup):
    """Replace the three DB-I/O helpers with in-memory stand-ins so
    ``score_broker_dealers`` can be exercised without a session."""
    from app.services import scoring as scoring_mod

    captured: dict = {}

    async def fake_load_target_firms(db, *, only_null_priority, limit):
        captured["only_null_priority"] = only_null_priority
        captured["limit"] = limit
        selected = [f for f in firms if (f.lead_priority is None) or not only_null_priority]
        if limit is not None:
            selected = selected[:limit]
        return selected

    async def fake_load_default_weights(db):
        return weights

    async def fake_load_competitor_lookup(db):
        return lookup

    monkeypatch.setattr(scoring_mod, "_load_target_firms", fake_load_target_firms)
    monkeypatch.setattr(scoring_mod, "_load_default_weights", fake_load_default_weights)
    monkeypatch.setattr(scoring_mod, "_load_competitor_lookup", fake_load_competitor_lookup)
    return captured


async def test_score_broker_dealers_filters_to_null_priority_by_default(
    monkeypatch, lookup, _hot_firm, _cold_firm
):
    """Default mode targets only rows with ``lead_priority IS NULL``.
    Pre-scored rows are excluded from the target set even when present
    in the simulated table."""
    from app.services.scoring import score_broker_dealers

    pre_scored = _FakeFirm(
        id=99,
        current_clearing_partner="Pershing LLC",
        clearing_classification="fully_disclosed",
        latest_net_capital=Decimal("50000000"),
    ...
    captured = _patch_loaders(
        monkeypatch,
        firms=[pre_scored, _hot_firm, _cold_firm],
        weights=_settings(),
        lookup=lookup,
    )

    summary = await score_broker_dealers(_FakeSession())

    assert captured["only_null_priority"] is True
    assert captured["limit"] is None
    assert summary.target_count == 2  # pre_scored excluded
    assert summary.scored == 2
    assert summary.skipped_no_data == 0
    assert summary.sample_ids == (1, 2)
    # Pre-scored row is untouched.
    assert pre_scored.lead_score == 85.0
    assert pre_scored.lead_priority == "hot"


async def test_score_broker_dealers_all_mode_rescores_everything(
    monkeypatch, lookup, _hot_firm, _cold_firm
):
    """``only_null_priority=False`` (the ``--all`` flag from the script)
    re-scores rows that already carry a priority. This is the post-w3
    refresh path: financials change, scores change."""
    from app.services.scoring import score_broker_dealers
    ...
    pre_scored = _FakeFirm(
        ...
        clearing_classification="fully_disclosed",
        latest_net_capital=Decimal("50000000"),
        lead_score=12.34,  # stale value from a prior run
        lead_priority="cold",  # stale priority that should be overwritten
    )
    _patch_loaders(
        monkeypatch,
        firms=[pre_scored, _hot_firm, _cold_firm],
    ...
    summary = await score_broker_dealers(_FakeSession(), only_null_priority=False)

    assert summary.target_count == 3
    assert summary.scored == 3
    # Pre-scored row gets refreshed.
    assert pre_scored.lead_score != 12.34
    assert pre_scored.lead_priority == "hot"


async def test_score_broker_dealers_skips_rows_with_no_scorable_data(
    monkeypatch, lookup, _hot_firm, _empty_firm
):
    """Per the brief: a BD with no financials, no clearing classification,
    and no competitor flags counts as ``skipped_no_data`` — we don't write
    a meaningless default-driven score."""
    from app.services.scoring import score_broker_dealers

    _patch_loaders(
        monkeypatch,
        firms=[_hot_firm, _empty_firm],
        weights=_settings(),
        lookup=lookup,
    )
    ...
    assert _hot_firm.lead_priority == "hot"
    # Skipped firm stays NULL — explicitly preserved.
    assert _empty_firm.lead_score is None
    assert _empty_firm.lead_priority is None


async def test_score_broker_dealers_dry_run_writes_nothing(
    monkeypatch, lookup, _hot_firm, _cold_firm
):
    """Dry-run reports the target set without invoking scoring."""
    from app.services.scoring import score_broker_dealers

    _patch_loaders(
        monkeypatch,
        firms=[_hot_firm, _cold_firm],
        weights=_settings(),
        lookup=lookup,
    )

    summary = await score_broker_dealers(_FakeSession(), dry_run=True)
    ...
    assert summary.sample_ids == (1, 2)
    # Nothing written.
    assert _hot_firm.lead_score is None
    assert _hot_firm.lead_priority is None
    assert _cold_firm.lead_score is None
    assert _cold_firm.lead_priority is None


async def test_score_broker_dealers_passes_limit_through(
    monkeypatch, lookup, _hot_firm, _cold_firm
):
    """The ``--limit`` flag from the script is forwarded to the loader."""
    from app.services.scoring import score_broker_dealers
    ...
    summary = await score_broker_dealers(_FakeSession(), limit=1)
...
async def test_score_broker_dealers_known_math(
    monkeypatch, lookup
):
    """Composite arithmetic sanity check against a known input through
    the bulk path — the same firm scored individually by
    ``calculate_lead_score`` should produce the same number when routed
    through ``score_broker_dealers``."""
    from app.services.scoring import score_broker_dealers
    ...
    firm = _FakeFirm(
        ...
        clearing_classification="omnibus",
        latest_net_capital=Decimal("15000000"),
        last_filing_date=today - timedelta(days=100),
        branch_count=10,
        is_niche_restricted=True,
    )
    _patch_loaders(
        monkeypatch,
        firms=[firm],
    ...
    summary = await score_broker_dealers(_FakeSession(), today=today)
    ...
    assert summary.scored == 1
    assert firm.lead_score == expected
    assert firm.lead_priority == classify_lead_priority(expected)
