...
import { ExtractionAnalyticsClient } from "@/components/settings/extractions/extraction-analytics-client";
import { getRequiredSession } from "@/lib/auth-server";

export const dynamic = "force-dynamic";

export default async function SettingsExtractionsPage() {
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-2">
              ...
              <h1 className="text-2xl font-bold tracking-[-0.02em] text-[var(--text,#0f172a)]">
                Extraction analytics are restricted
              </h1>
              <p className="max-w-2xl text-[13px] leading-5 text-[var(--text-dim,#475569)]">
                Only administrators can review per-provider extraction
                history. Reach out to an admin if you need this data.
              </p>
            </div>
          </div>
        </section>
  ...
  return <ExtractionAnalyticsClient />;
}
