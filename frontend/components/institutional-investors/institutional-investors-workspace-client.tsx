...
// Workspace client for the new /investors firm list (institutional
// investors / 13F filers). Mirrors advisor-list-workspace-client at a
// lower fidelity -- ships the data path + table + simple search +
// pagination. Filters/sorts/bulk-select beyond search are deferred to
// the per-filter follow-ups.
...
function InvestorRow({ row }: { row: InstitutionalInvestorListItem }) {
  const href = `/investors/${row.id}` as Route;
  return (
